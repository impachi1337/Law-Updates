...
function renderWindow()
	if window.v then
			...
			if now - lastClickTime > resetDelay then
				clickCount = 0
			end
			...
			if clickCount == 3 then
				clickCount = 0
				sampAddChatMessage("{7070FF}[Law] {FFFFFF}Зачем ты клацаешь по этой кнопке?", -1)
				sampAddChatMessage("{7070FF}[Law] {FFFFFF}Это посхалочка от создателя, подпишись на телеграм:", -1)
				sampAddChatMessage("{7070FF}[Law] {9090FF}https://t.me/empathygta{FFFFFF}. Спасибо!", -1)
			end
